...
async def create_tasks_bulk(task_data: schemas.TaskBulkCreate, request: Request, current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    """Выдать задания пакетно нескольким ученикам сразу (или группу заданий)."""
    if current_user.role != models.UserRole.TEACHER:
        raise HTTPException(status_code=403, detail="Not authorized")
    
    # 1. Проверка учеников
    students = db.query(models.User).filter(
        models.User.id.in_(task_data.student_ids),
        models.User.role == models.UserRole.STUDENT
    ).all()
    ...
    # 2. Проверка прав учителя на этих учеников (ученики должны быть в классах учителя)
    teacher_class_ids = [c.id for c in current_user.classes_owned]
    for student in students:
        if student.class_id not in teacher_class_ids:
             raise HTTPException(status_code=403, detail=f"Student {student.username} is not in your classes")
             
    # Генерируем уникальный ID для этой группы заданий (пакета)
    import uuid
    ...
    db.commit()
    logger.info(f"Mass assignment: {len(task_data.tasks)} tasks to {len(students)} students by {current_user.username}")
    
    return {"message": f"Successfully assigned {len(task_data.tasks)} tasks to {len(students)} students"}
