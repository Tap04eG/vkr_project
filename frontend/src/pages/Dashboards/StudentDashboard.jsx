...
const StudentTasks = ({ tasks, onStartTask }) => {
    ...
    // 2. Helper to render a single task card
    const renderTaskCard = (task, isGrouped = false) => (
        <div key={task.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', opacity: task.status === 'completed' ? 0.7 : 1, border: isGrouped ? 'none' : '', borderBottom: isGrouped ? '1px solid #eee' : '', borderRadius: isGrouped ? '0' : '', boxShadow: isGrouped ? 'none' : '', padding: isGrouped ? '15px 0' : '' }}>
            <div>
                <h3 style={{ marginBottom: '0.5rem', fontSize: isGrouped ? '1.1rem' : '1.25rem', textDecoration: task.status === 'completed' ? 'line-through' : 'none' }}>
                    {task.title}
                </h3>
                <p style={{ color: 'var(--text-light)', margin: 0 }}>{task.description}</p>
                <span style={{ display: 'inline-block', marginTop: '0.5rem', padding: '4px 12px', background: '#f0fdf6', borderRadius: '20px', fontSize: '0.85rem', color: 'var(--primary-dark)' }}>
            ...
            {task.status === 'completed' ? (
                ...
                <button
                    ...
                    className={isGrouped ? "btn btn-secondary" : "btn btn-primary"}
        ...
        <div>
            ...
            <div style={{ display: 'grid', gap: '1.5rem' }}>
                {tasks.length === 0 ? (
                    ...
                    <>
                        {/* Render Task Groups (Test Packs) */}
                        {Object.entries(groupedTasks).map(([groupId, groupTasks]) => {
                            const totalReward = groupTasks.reduce((sum, t) => sum + t.reward_xp, 0);
                            const completedCount = groupTasks.filter(t => t.status === 'completed' || t.status === 'on_review').length;
                            const nextPending = groupTasks.find(t => t.status === 'pending');

                            return (
                                <div key={groupId} className="card" style={{ borderLeft: '4px solid #6366f1', padding: '20px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', paddingBottom: '15px', borderBottom: '2px solid #f0f0f0' }}>
                                        <div>
                                            <h3 style={{ margin: 0, color: '#4338ca', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <BookOpen size={20} /> Пакет заданий (Тест)
                                            </h3>
                                            <div style={{ fontSize: '0.9rem', color: '#666', marginTop: '5px' }}>
                                                Прогресс: {completedCount} / {groupTasks.length} | Общая награда: {totalReward} XP
                                            </div>
                                        </div>
                                        {nextPending ? (
                                            <button onClick={() => onStartTask(nextPending)} className="btn btn-primary" style={{ padding: '10px 25px' }}>
                                                Начать тест 🚀
                                            </button>
                                        ) : (
                                            <span style={{ color: '#10b981', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}>
                                                <CheckCircle /> Тест завершен
                                            </span>
                                        )}
                                    </div>
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        {groupTasks.sort((a, b) => a.id - b.id).map(t => renderTaskCard(t, true))}
                                    </div>
                                </div>
                            );
                        })}
                        ...
                        {/* Render Single Tasks */}
                        {singleTasks.map(task => renderTaskCard(task, false))}
                    </>
                )}
            </div>
        </div>
    ...
    const handleComplete = async (taskId, resultData) => {
        try {
            ...
            if (resultData && resultData.correct) {
                ...
                nextLevel = Math.floor(resultData.new_xp / 100) + 1;
                currentXp = resultData.new_xp;
                newTaskStatus = resultData.status || 'completed';
            } else {
                const res = await api.post(`/tasks/${taskId}/complete`);
                alert(`Задание выполнено! +${res.data.new_xp - user.xp} XP`);
                nextLevel = res.data.new_level;
                currentXp = res.data.new_xp;
            }

            setUser({ ...user, xp: currentXp, level: nextLevel });

            const updatedTasks = tasks.map(t => t.id === taskId ? { ...t, status: newTaskStatus } : t);
            setTasks(updatedTasks);

            // AUTO-ADVANCEMENT LOGIC for grouped tasks
            const completedTask = tasks.find(t => t.id === taskId);
            if (completedTask && completedTask.group_id) {
                // Find next pending task in the same group
                const groupTasks = updatedTasks.filter(t => t.group_id === completedTask.group_id);
                // Sort by ID to keep order
                groupTasks.sort((a, b) => a.id - b.id);
                const nextTask = groupTasks.find(t => t.status === 'pending');
                ...
                if (nextTask) {
                    ...
                    return; // exit early, keep modal open with new task
                }
            }

            // If no next task in group, close modal
            setActiveTask(null);
            ...
            {activeTask && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <StudentTaskPlayer
                            key={activeTask.id} // Add key to force remount on task switch
                            task={activeTask}
                            onComplete={(score) => {
                                handleComplete(activeTask.id, score);
                                // Removed setActiveTask(null) from here; handled inside handleComplete
                            }}
                        />
                    </div>
                </div>
